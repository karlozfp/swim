...
# Connect to database -----------------------------------------------------
  ...
  # ============ HWSD DATA INTO SWIM package
  # hwsd <- dplyr::tbl(con.hwsd, input$hwsd.soil.data) %>%
  ...
  #   ) %>%
  #   dplyr::collect() %>% data.frame()# hwsd data
  # SU90 <- dplyr::tbl(con.hwsd, "D_SYMBOL90") %>% dplyr::collect() %>% data.frame()
  # SU85 <- dplyr::tbl(con.hwsd, "D_SYMBOL85") %>% dplyr::collect() %>% data.frame()
  # SU74 <- dplyr::tbl(con.hwsd, "D_SYMBOL74") %>% dplyr::collect() %>% data.frame()
  #
  # hwsd_data <- list(HWSD_DATA = hwsd, D_SYMBOL90 = SU90, D_SYMBOL85 = SU85, D_SYMBOL74 = SU74)
  # use_data(hwsd_data)
  # ============

  # hwsd data
  hwsd <- SWIM::hwsd_data$HWSD_DATA # hwsd data
  SU90 <- SWIM::hwsd_data$D_SYMBOL90
  SU85 <- SWIM::hwsd_data$D_SYMBOL85
  SU74 <- SWIM::hwsd_data$D_SYMBOL74
...
# function to write soil files
  ...
  { #	The function will produce line by line required for the soil file.
    ...
    # write to file
    write.table(d, f_con, quote=FALSE, col.names=FALSE, row.names=FALSE, sep = " ")
